...
#define LOCKDOCKD_LAUNCHAGENT_LABEL "co.myrt.lockdockd"

static void lockdockd_set_message(char *buffer,
                                  size_t buffer_size,
                                  const char *message) {
...
static bool lockdockd_append_cstring(char *buffer,
                                     size_t buffer_size,
                                     size_t *used,
                                     const char *text) {
    if (text == NULL) {
    ...
    return lockdockd_append_bytes(buffer, buffer_size, used, text, strlen(text));
}

static bool lockdockd_append_xml_escaped(char *buffer,
                                         size_t buffer_size,
                                         size_t *used,
                                         const char *text) {
    while (text != NULL && *text != '\0') {
        ...
        if (replacement != NULL) {
            if (!lockdockd_append_cstring(buffer, buffer_size, used, replacement)) {
                return false;
            }
        } else if (!lockdockd_append_bytes(buffer, buffer_size, used, text, 1)) {
            return false;
        }
...
static void lockdockd_trim_trailing_whitespace(char *text) {
    size_t length;
...
static bool lockdockd_copy_launchagents_dir(char *buffer,
                                            size_t buffer_size,
                                            char *error,
                                            size_t error_size) {
    ...
    if (home == NULL) {
        lockdockd_set_message(error, error_size,
...
static bool lockdockd_copy_plist_path(char *buffer,
                                      size_t buffer_size,
                                      char *error,
                                      size_t error_size) {
    char directory[PATH_MAX];

    if (!lockdockd_copy_launchagents_dir(directory, sizeof(directory), error,
                                         error_size)) {
        return false;
    }

    if (snprintf(buffer, buffer_size, "%s/%s.plist", directory,
                 LOCKDOCKD_LAUNCHAGENT_LABEL) >= (int)buffer_size) {
        lockdockd_set_message(error, error_size,
...
static bool lockdockd_copy_executable_path(char *buffer,
                                           size_t buffer_size,
                                           char *error,
                                           size_t error_size) {
    ...
    if (_NSGetExecutablePath(raw_path, &raw_size) != 0) {
        lockdockd_set_message(error, error_size, "Executable path is too long");
        return false;
    }
...
static bool lockdockd_copy_domain_target(char *buffer,
                                         size_t buffer_size,
                                         char *error,
                                         size_t error_size) {
...
static bool lockdockd_copy_service_target(char *buffer,
                                          size_t buffer_size,
                                          char *error,
                                          size_t error_size) {
...
static bool lockdockd_build_plist(char *buffer,
                                  size_t buffer_size,
    ...
    if (buffer == NULL || executable_path == NULL) {
        lockdockd_set_message(error, error_size, "Internal error");
        return false;
    }

    buffer[0] = '\0';

    if (!lockdockd_append_cstring(
            ...
            "    <key>Label</key>\n"
            "    <string>") ||
        !lockdockd_append_xml_escaped(buffer, buffer_size, &used,
                                      LOCKDOCKD_LAUNCHAGENT_LABEL) ||
        !lockdockd_append_cstring(buffer, buffer_size, &used,
                                  "</string>\n"
                                  "    <key>ProgramArguments</key>\n"
                                  "    <array>\n"
                                  "        <string>") ||
        !lockdockd_append_xml_escaped(buffer, buffer_size, &used, executable_path) ||
        !lockdockd_append_cstring(buffer, buffer_size, &used,
                                  "</string>\n"
                                  "    </array>\n"
                                  "    <key>RunAtLoad</key>\n"
                                  ...
                                  "</dict>\n"
                                  "</plist>\n")) {
        lockdockd_set_message(error, error_size, "LaunchAgent plist is too large");
        return false;
    }
...
static bool lockdockd_write_plist(const char *plist_path,
    ...
    if (!lockdockd_write_all(fd, plist_content, strlen(plist_content), error,
                             error_size)) {
        goto cleanup;
    }
...
static bool lockdockd_read_command_output(int fd, char *buffer, size_t buffer_size) {
    size_t used = 0;
...
static bool lockdockd_run_command(const char *const *argv,
                                  int *exit_status,
                                  char *output,
                                  size_t output_size,
                                  char *error,
    ...
    if (!lockdockd_read_command_output(pipefd[0], output, output_size)) {
        close(pipefd[0]);
    ...
    if (output != NULL && output_size > 0) {
        lockdockd_trim_trailing_whitespace(output);
    }
...
static bool lockdockd_launchctl(char *error,
                                size_t error_size,
                                const char *action,
                                const char *const *argv) {
    char output[LOCKDOCKD_LAUNCHAGENT_MESSAGE_SIZE];
    int exit_status = 0;

    if (!lockdockd_run_command(argv, &exit_status, output, sizeof(output), error,
                               error_size)) {
        return false;
    }
...
static bool lockdockd_launchctl_best_effort(const char *const *argv,
                                            char *error,
                                            size_t error_size) {
    char output[LOCKDOCKD_LAUNCHAGENT_MESSAGE_SIZE];
    int exit_status = 0;
...
bool lockdockd_launchagent_enable(char *message, size_t message_size) {
    ...
    char error[LOCKDOCKD_LAUNCHAGENT_MESSAGE_SIZE];
    ...
    if (!lockdockd_copy_executable_path(executable_path, sizeof(executable_path),
                                        error, sizeof(error)) ||
        !lockdockd_copy_launchagents_dir(directory, sizeof(directory), error,
                                         sizeof(error)) ||
        !lockdockd_copy_plist_path(plist_path, sizeof(plist_path), error,
                                   sizeof(error)) ||
        !lockdockd_copy_domain_target(domain_target, sizeof(domain_target), error,
                                      sizeof(error)) ||
        !lockdockd_copy_service_target(service_target, sizeof(service_target), error,
                                       sizeof(error))) {
        lockdockd_set_message(message, message_size, error);
        return false;
    }

    if (!lockdockd_mkdir_p(directory, error, sizeof(error)) ||
        !lockdockd_build_plist(plist_content, sizeof(plist_content), executable_path,
                               error, sizeof(error)) ||
        !lockdockd_write_plist(plist_path, plist_content, error, sizeof(error))) {
        lockdockd_set_message(message, message_size, error);
        return false;
    }

    if (!lockdockd_launchctl_best_effort(enable_argv, error, sizeof(error)) ||
        !lockdockd_launchctl_best_effort(bootout_argv, error, sizeof(error)) ||
        !lockdockd_launchctl(error, sizeof(error), "launchctl bootstrap",
                             bootstrap_argv)) {
        lockdockd_set_message(message, message_size, error);
        return false;
    }
...
bool lockdockd_launchagent_disable(char *message, size_t message_size) {
    ...
    if (!lockdockd_copy_plist_path(plist_path, sizeof(plist_path), error,
                                   sizeof(error)) ||
        !lockdockd_copy_service_target(service_target, sizeof(service_target), error,
                                       sizeof(error))) {
        lockdockd_set_message(message, message_size, error);
    ...
    if (!lockdockd_launchctl(error, sizeof(error), "launchctl disable",
                             disable_argv) ||
        !lockdockd_launchctl_best_effort(bootout_argv, error, sizeof(error))) {
        lockdockd_set_message(message, message_size, error);
        return false;
